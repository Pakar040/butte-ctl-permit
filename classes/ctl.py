...
class CTLXLSX:
    ...
    def _load_template_xls(self):
        self.workbook = xw.Book(self.template_filepath)
        self.active_sheet = self.workbook.sheets[0]
    ...
    def write_all_data_to_workbook(self):
        ...
        for index, ctl_pole in enumerate(self.pole_data_list):
            if counter == 15:
                ...
                if page == len(self.workbook.sheets):
                    print("OUT OF ROWS!!!")
                    break

                self.active_sheet = self.workbook.sheets[page]
                counter = 0
